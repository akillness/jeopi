...
| **"Done"** | tests pass | **artifact gate + independent grader** — a criterion with no command + observed result is reported `unresolved`, never implied met. `goal({op:"complete"})` runs two checks in sequence, both **runtime-enforced**, neither skippable by the same agent that did the work: (1) a deterministic evidence gate — rejected unless a test/build/typecheck/lint run succeeded after the last file mutation; (2) a `goal-verifier` sub-agent — fresh context, no exposure to the maker's transcript/thinking, re-inspects the diff and re-runs the claimed verification independently, verdicts `okay`/`iterate`/`reject`. Either gate bounces once with a corrective message before an unchanged retry passes (docs-only / verifier-false-positive escape hatch); any new mutation or verification evidence re-arms both |
...
- **[16.5.0]** (2026-08-19) — Resuming a session whose previous process died mid-turn no longer replays a non-terminal transcript to the provider. An abnormal exit recorded after an unanswered tool call (or a first-turn user…
- **[16.4.3]** (2026-07-22) — `jeopi acp` now prints a short hint on stderr when launched from an interactive terminal (stdin is a TTY): the command speaks JSON-RPC over stdout and is meant to be spawned by an ACP client such as…
- **[16.4.2]** (2026-07-14) — `AgentSession`'s `reasoning_extraction` classifier-refusal auto-degrade (strip unsigned thinking, resend same-model) was gated behind a one-shot latch that, once spent on the first refusal in a…
- **[16.4.1]** (2026-07-14) — The interrupted-thinking continuity message (`Esc` mid-stream on a turn with unsigned thinking, replayed on the next turn's resume so the model can continue where it left off) shipped the model's own…
- **[16.4.0]** (2026-07-12) — The `learn` tool now accepts optional `verified`/`evidence` params (article gap: "verified fact" vs. "hypothesis" distinction). Every `learned.md` line is now tagged `[VERIFIED]`/`[UNVERIFIED]`…
