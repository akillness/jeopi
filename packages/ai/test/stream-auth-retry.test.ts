...
import type { ApiKeyResolveContext } from "jeopi-ai";
import { registerCustomApi, unregisterCustomApis } from "jeopi-ai";
import { streamSimple } from "jeopi-ai/stream";
import type { Api, AssistantMessage, Context, Model, SimpleStreamOptions, Usage } from "jeopi-ai/types";
...
describe("streamSimple resolver auth retry", () => {
	...
	it("retries with a refreshed key when a 401 is thrown before the first event", async () => {
		...
		const secondContextError = contexts[1]!.error as { status?: number };
		expect(secondContextError.status).toBe(401);
	});
	...
	it("rotates on the exact Google Resource exhausted 429 error before content", async () => {
		...
		const secondRetryError = retryContexts[1]!.error as Error;
		expect(secondRetryError.message).toContain("Resource exhausted");
	});
