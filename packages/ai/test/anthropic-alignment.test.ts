...
describe("Anthropic request fingerprint alignment", () => {
	...
	it("does not place cache_control on thinking blocks in the trailing cache window", async () => {
		...
		// Test-internal capture array; content shape is asserted, not schema-validated.
		const lastContent = last!.content as Array<{ cache_control?: unknown }>;
		expect(lastContent[0]?.cache_control).toBeDefined();
	});
