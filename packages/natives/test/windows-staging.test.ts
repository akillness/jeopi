 ...
 * `node_modules/jeopi-natives/native/pi_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.jeopi/natives/<version>/`) so the
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.jeopi\\natives\\15.0.1";
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.jeopi/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
