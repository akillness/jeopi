...
- Added package-level TypeScript exports and publication config so consumers can import `jeopi-snapcompact` with typed access to snapcompact APIs
- Published `jeopi-snapcompact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
