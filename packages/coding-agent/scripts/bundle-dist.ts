...
// (native addon, mupdf).
const ALWAYS_EXTERNAL = ["mupdf", "jeopi-natives", "@huggingface/transformers", "fastembed", "onnxruntime-node"];
...
async function cleanBundleOutputs(): Promise<void> {
	// dist/ is shared with the dev binary (dist/jeopi); only remove this
