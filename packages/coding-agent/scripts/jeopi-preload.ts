/**
 * Bun `--preload` shim for the jeopi dev launcher (`scripts/jeopi`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the jeopi CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `jeopi-utils/dirs`).
 */
const launchCwd = process.env.JEOPI_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.JEOPI_LAUNCH_CWD;
	try {
