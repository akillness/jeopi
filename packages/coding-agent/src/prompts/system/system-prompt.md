...
- Vague ask? Crystallize goal, constraints, and checkable acceptance criteria before planning — sharpen "make it better" into something mechanically checkable instead of accepting it.
...
- Keep a running picture — goal/constraints, confirmed evidence, failed approaches and why, open candidates — and update it as you learn instead of re-deriving it from history.

# 4. Implement
- Fix problems at the source. Remove obsolete code—no leftover comments, aliases, or re-exports.
- A failed tool call or test is information: capture what it proved, change the next attempt accordingly, and split a stuck subgoal smaller rather than retrying unchanged. No apology loops.
