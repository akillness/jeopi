...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.jeopi/, .pi/, .claude/)
	...
	// User config directories (~/.jeopi/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .jeopi/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.jeopi/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
