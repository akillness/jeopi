 ...
 */
import type { CommandEntry } from "jeopi-utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `jeopi list` to
// `jeopi launch list`, silently forwarding the bare verb to the model as a prompt
...
// `jeopi plugin <action>` command; multi-word invocations still fall through to
...
const RESERVED_TOP_LEVEL_WORDS = new Map<string, string>([
	[
		"extensions",
		'`jeopi extensions` is not a management command. Use `jeopi plugin list` / `jeopi plugin install`, or run `jeopi launch extensions` if you meant to send "extensions" as a prompt.',
	],
	[
		"list",
		'`jeopi list` is not a top-level command. Use `jeopi plugin list` to list installed plugins, or run `jeopi launch list` if you meant to send "list" as a prompt.',
	],
	[
		"remove",
		'`jeopi remove` is not a top-level command. Use `jeopi plugin uninstall <name>` to remove a plugin, or run `jeopi launch remove` if you meant to send "remove" as a prompt.',
	],
]);
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`jeopi --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
