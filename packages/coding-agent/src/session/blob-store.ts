...
/** Canonical blob suffix: a 64-char lowercase hex SHA-256 digest, nothing else. */
 ...
 * `get`/`getSync`/`has` feed this value into `path.join(this.dir, hash)`, so an
 * unvalidated `../` suffix in a crafted session file would otherwise escape the
 * store and read arbitrary files.
 */
