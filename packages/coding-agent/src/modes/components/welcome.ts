...
/**
 * Premium welcome screen with the block-glyph jeopi wordmark and two-column layout.
 */
export class WelcomeComponent implements Component {
	...
	#renderLines(termWidth: number): string[] {
		...
		const preferredLeftCol = 27;
		const minLeftCol = 23; // logo width
		...
		const leftLines = [
			...
			"",
			this.#centerText(theme.fg("dim", TAGLINE_LINE_1), leftCol),
			this.#centerText(theme.fg("dim", TAGLINE_LINE_2), leftCol),
			"",
...
/**
 * "jeopi" block-glyph wordmark — typography only (per brand: ASCII surfaces
 * carry the wordmark as pure type, no mascot art). 6 rows: r0 dots, r1–r4
 * x-height, r5 descenders (j hook, p stem). Rendered through {@link gradientLogo}
 * so the terminal mark carries the same neon blue→violet→pink synthwave sweep
 * as the webp wordmark on the OAuth callback page (jeo-code mascot palette).
 */
export const JEOPI_LOGO = [
	"  ██                 ██",
	"  ▄▄ ████ ▄███▄ ████ ▄▄",
	"  ██ ██ █ ██ ██ ██ █ ██",
	"  ██ ████ ██ ██ ██ █ ██",
...
/** Persona tagline (jeo-code identity), split to fit the welcome left column. */
export const TAGLINE_LINE_1 = "Encode intention.";
export const TAGLINE_LINE_2 = "Decode software.";

/** Multi-stop palette for the diagonal gradient: electric blue → hot pink,
 *  left-to-right — matches the jeopi webp wordmark and jeo-code's mascot
 *  (blue lens → violet gown → pink lens). */
const GRADIENT_STOPS: ReadonlyArray<readonly [number, number, number]> = [
	[60, 160, 255], // electric blue
...
const GRADIENT_RAMP_256 = [39, 75, 99, 135, 171, 199, 205];
...
function introLogoFrame(progress: number): string[] {
	...
	return gradientLogo(JEOPI_LOGO, phase, { strength: shineStrength, pos: shinePos });
}

/** Resting gradient frame, cached for re-renders outside of the intro. */
const REST_FRAME = gradientLogo(JEOPI_LOGO, 0);
