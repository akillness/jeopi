import { padding, truncateToWidth, visibleWidth } from "jeopi-tui";
import { gradientEscape, gradientLogo, JEOPI_LOGO, type ShineConfig } from "../../components/welcome";
...
const LARGE_LOGO = JEOPI_LOGO.flatMap(line => {
	let wide = "";
...
/**
 * Animated setup splash, in the spirit of the jeopi landing page: the brand
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	// 2x mark is 12 rows; +1 gap +1 wordmark +2 hint rows ⇒ needs ≥ 17 rows.
	const art = height >= 17 ? LARGE_LOGO : JEOPI_LOGO;
	const content = [...gradientLogo(art, phase, shine), "", theme.bold("j e o p i")];
