 ...
 */
import { getProjectDir, getSSHConfigPath } from "jeopi-utils";
	...
	async #handleList(): Promise<void> {
		try {
			...
			if (userHosts.length > 0) {
				lines.push(theme.fg("accent", "User level") + theme.fg("muted", ` (~/.jeopi/agent/ssh.json):`));
			...
			if (projectHosts.length > 0) {
				lines.push(theme.fg("accent", "Project level") + theme.fg("muted", ` (.jeopi/ssh.json):`));
