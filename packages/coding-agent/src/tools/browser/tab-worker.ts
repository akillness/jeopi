...
import { Snowflake, untilAborted } from "jeopi-utils";
...
import { type WaitPredicateOptions, waitForBrowserRun } from "./run-cancellation";
...
export class WorkerCore {
	...
	#createTabApi(
		...
		): Promise<T> => this.#runOp(active, label, signal, perOpMs, fn, selectorOpts);
		return {
			name,
			page,
			signal,
