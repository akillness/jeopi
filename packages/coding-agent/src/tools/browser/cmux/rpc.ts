...
export function serializeEvalWithEnvelope(fn: string | ((...args: unknown[]) => unknown), args: unknown[]): string {
	...
	return `(() => {
		try {
			const __v = (${expr});
			if (__v && typeof __v.then === "function") return { __jeopiPromise: true };
			return { __jeopiOk: __v === undefined ? null : __v };
		} catch (e) {
			return { __jeopiErr: (e && (e.stack || e.message)) || String(e) };
		}
	})()`;
}
...
export function unwrapEvalEnvelope<TResult>(value: unknown, label: string): TResult {
	if (value && typeof value === "object") {
		if ("__jeopiErr" in value && typeof value.__jeopiErr === "string") {
			throw new ToolError(`${label} threw a JavaScript exception:\n${value.__jeopiErr}`);
		}
		if ("__jeopiPromise" in value && value.__jeopiPromise === true) {
			throw new ToolError(
		...
		if ("__jeopiOk" in value) {
			return value.__jeopiOk as TResult;
		}
	}
