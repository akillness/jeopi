 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.jeopi.sh`). Each
 ...
 * `~/.jeopi/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `jeopi grievances push`, future debug scripts —
 ...
 *
 * Exported because the `jeopi grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `jeopi grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`jeopi grievances push`) where the
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.jeopi/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `jeopi grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `jeopi grievances push` or a future consent change.
