...
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "jeopi-utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `JEOPI_NO_WEBP=1`.
 */
 ...
 * `JEOPI_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `JEOPI_NO_WEBP`. */
	excludeWebP?: boolean;
}
