...
import { FileType, glob } from "jeopi-natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ jeopi?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.jeopi ?? pkg?.pi;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "jeopi"/"pi" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.jeopi/`.
 *    The first match returns `<dir>/.jeopi/plugins/installed_plugins.json`.
 * 2. If no `.jeopi/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.jeopi/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .jeopi/ directory (nearest wins).
	// Stop before os.homedir() — ~/.jeopi/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.jeopi/plugins/installed_plugins.json`
 * when no project anchor (.jeopi/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .jeopi/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.jeopi/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	if (ompContent) {
		...
		if (ompRegistry) {
			...
			warnings.push(`Failed to parse jeopi plugin registry: ${ompRegistryPath}`);
		}
	}

	// ── Project-scoped OMP registry ────────────────────────────────────────
	// Loaded from the nearest .jeopi/plugins/installed_plugins.json relative to cwd.
