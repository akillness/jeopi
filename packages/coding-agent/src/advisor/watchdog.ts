...
import { CONFIG_DIR_NAME, getAgentDir, isEnoent, logger, prompt } from "jeopi-utils";
 ...
 * `.jeopi/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.jeopi/agent/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .jeopi folder, and user agent dir.
