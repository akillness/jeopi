...
export default class DryBalance extends Command {
	...
	static flags = {
		model: Flags.string({ description: "Model selector (same syntax as --model on jeopi)" }),
	...
	static examples = [
		...
		"# Dry-run a specific model\n  jeopi dry-balance anthropic/claude-sonnet-4-5",
		"# Larger run with bounded concurrency\n  jeopi dry-balance --model openai-codex/gpt-5-codex --count 1000 --concurrency 64",
		"# Benchmark every OAuth account in parallel\n  jeopi dry-balance --bench",
		"# Machine-readable output\n  jeopi dry-balance --json",
	];
