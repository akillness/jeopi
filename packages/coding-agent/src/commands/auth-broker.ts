...
export default class AuthBroker extends Command {
	static description = "Manage the jeopi auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  jeopi auth-broker serve",
		"# Boot on a non-default port\n  jeopi auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  jeopi auth-broker token",
		"# Rotate the bearer token\n  jeopi auth-broker token --regenerate",
		"# List supported OAuth providers\n  jeopi auth-broker list",
		"# Local login (run on the broker host)\n  jeopi auth-broker login anthropic",
		"# Interactive provider selection\n  jeopi auth-broker login",
		"# Remote login over SSH tunnel\n  jeopi auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  jeopi auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  jeopi auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  jeopi auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  jeopi auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  jeopi auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  jeopi auth-broker status",
	];
