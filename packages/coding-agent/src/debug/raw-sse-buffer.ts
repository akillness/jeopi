import type { Model, ProviderResponseMetadata, RawSseEvent } from "jeopi-ai";
...
function trimRawLines(raw: string[]): TrimResult {
	...
	const tail = `: jeopi-debug-truncated originalChars=${originalChars}`;
	trimmed.push(tail);
	chars += tail.length + 1;
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: jeopi-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
