...
import type { WorkProfile } from "jeopi-natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "jeopi-utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `jeopi-report-${timestamp}.tar.gz`);
