...
import { getMCPConfigPath, logger } from "jeopi-utils";
...
async function handleSmitherySearchCommand(rest: string, runtime: SlashCommandRuntime): Promise<SlashCommandResult> {
	...
	try {
		...
		if (AIError.is(AIError.classify(err), AIError.Flag.AuthFailed)) {
			return usage(
				"Smithery authentication required. Run /mcp smithery-login in the TUI client or add an API key to ~/.jeopi/agent/smithery.json.",
				runtime,
			);
		}
