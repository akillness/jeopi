...
import { $which, APP_NAME, isEnoent, VERSION } from "jeopi-utils";
 ...
 * (`react@19.2.6@@@1`). Global `jeopi` updates can leave one full copy per
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(chalk.yellow(`You may need to reinstall: bun install -g jeopi`));
}
 ...
 *
 * Together these two flags make `jeopi update` produce exactly the registry
 * lookup the version check just performed. See #1686.
 *
 * Also pins {@link NATIVES_PACKAGE} and the platform-specific
 * `jeopi-natives-<tag>` leaf to `expectedVersion`. `bun install -g`
 ...
 * `jeopi` and the loader aborts at
