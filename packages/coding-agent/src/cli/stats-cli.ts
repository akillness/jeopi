 ...
 *
 * Handles `jeopi stats` subcommand for viewing AI usage statistics.
 */

import chalk from "chalk";
import { truncateToWidth } from "jeopi-tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "jeopi-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("jeopi-stats");
	const stats = await getDashboardStats();
