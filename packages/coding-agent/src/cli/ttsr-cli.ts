 ...
 *
 * `jeopi ttsr test` — feed a snippet (inline text, `--file`, or stdin) through the
 ...
 *
 * `jeopi ttsr list` — show every TTSR-registered rule the current project/user
 * config would load, with its conditions, scope, and source.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import chalk from "chalk";
import { AstMatchStrictness, astMatch, FileType, type GlobMatch, glob } from "jeopi-natives";
import { getProjectDir } from "jeopi-utils/dirs";
