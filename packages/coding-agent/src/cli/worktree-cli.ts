/**
 * CLI handler for `jeopi worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.jeopi/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "jeopi-utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.jeopi/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `jeopi worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.jeopi/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
