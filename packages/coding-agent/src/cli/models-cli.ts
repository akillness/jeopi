/**
 * `jeopi models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.jeopi/models.db`
...
import type { Api, Effort, Model } from "jeopi-ai";
import { getSupportedEfforts } from "jeopi-catalog/model-thinking";
import { formatNumber, getProjectDir } from "jeopi-utils";
 ...
 * name doubles as an `jeopi models <provider>` shortcut.
 */
...
/** `jeopi models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `jeopi models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`jeopi models find` requires a search substring, e.g. `jeopi models find minimax`\n");
		process.exitCode = 1;
		return;
	}
