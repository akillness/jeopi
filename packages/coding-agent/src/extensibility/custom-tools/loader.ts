...
async function loadTool(
	...
	try {
		...
		const toolResult: unknown = await factory(sharedApi);
 ...
 * `.jeopi/tools/`, `.claude/tools/`, the plugin tree, and any configured paths.
 *
...
export async function discoverCustomToolPaths(configuredPaths: string[], cwd: string): Promise<ToolPathWithSource[]> {
	...
	// 2. Plugin tools: ~/.jeopi/plugins/node_modules/*/
 ...
 * 2. Installed plugins (~/.jeopi/plugins/node_modules/*)
