...
import type { CustomToolContext } from "jeopi-cli/extensibility/custom-tools";
import { DeferredMCPTool, MCPTool, type MCPToolDefinition } from "jeopi-cli/mcp";
import type { MCPServerConnection } from "jeopi-cli/mcp/types";
...
async function createLocalImageContext(
	...
	// macOS's TMPDIR is itself a symlink (/var -> /private/var), so the path
	// must be realpath'd the same way resolveLocalUrlToFile's own containment
	// check does, or the raw expectation mismatches by a "/private" prefix.
