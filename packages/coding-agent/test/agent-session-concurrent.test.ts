...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "jeopi-agent-core";
import type { AssistantMessage, Message, ToolCall } from "jeopi-ai";
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".jeopi", "rules", "no-unwrap.md");
