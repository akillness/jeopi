...
import { buildSpec, type CompletionSpec, generateCompletion } from "jeopi-cli/cli/completion-gen";
import type { CliConfig, CommandCtor } from "jeopi-utils/cli";
...
describe("omp completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		...
		expect(stdout).toContain("command jeopi __complete $kind");
