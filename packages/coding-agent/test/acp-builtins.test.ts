...
import type { ResetCreditAccountStatus, ResetCreditRedeemOutcome, ResetCreditTarget, UsageReport } from "jeopi-ai";
import { Settings } from "jeopi-cli/config/settings";
import type { AgentSession } from "jeopi-cli/session/agent-session";
import type { SessionManager } from "jeopi-cli/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "jeopi-cli/slash-commands/acp-builtins";
import { removeWithRetries, setProjectDir } from "jeopi-utils";
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		// Uses project scope so it writes to /tmp/project/.jeopi/mcp.json which test infra controls.
		// We verify the command either reports success or a meaningful error (not a parse error).
		const mcpModule = await import("jeopi-cli/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("jeopi-cli/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("jeopi-cli/extensibility/plugins/marketplace");
