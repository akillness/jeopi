...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "jeopi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.jeopi/`), plus
//    the legacy non-XDG `~/.jeopi/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.jeopi/agent` or `$PI_CODING_AGENT_DIR`)
...
//    Handles installed plugin extensions that live outside `~/.jeopi` when
...
// symlinks `~/.jeopi/agent/extensions/foo.ts` to a dotfiles repo would see the
