...
import { substitutePluginRoot } from "jeopi-cli/discovery/substitute-plugin-root";
...
const JEOPI_VAR = "$" + "{JEOPI_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces JEOPI_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${JEOPI_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${JEOPI_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
