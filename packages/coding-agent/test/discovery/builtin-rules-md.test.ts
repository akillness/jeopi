 ...
 * from both `~/.jeopi/agent/RULES.md` (user) and the nearest `.jeopi/RULES.md`
...
import { type Rule, ruleCapability } from "jeopi-cli/capability/rule";
...
test("user ~/.jeopi/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".jeopi", "agent", "RULES.md"),
...
test("project .jeopi/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".jeopi", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".jeopi", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".jeopi", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".jeopi", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .jeopi/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".jeopi", "agent", "rules", "other.md"), "# Unrelated rule\n");
