...
import { getConfigDirs } from "jeopi-cli/config";
import { getUserPath } from "jeopi-cli/discovery/helpers";
import { getAgentDir } from "jeopi-utils";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".jeopi/agent" and leaked the default
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".jeopi", level: "user" });
	});
});
