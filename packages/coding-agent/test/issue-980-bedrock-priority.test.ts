...
import { resolveCliModel, resolveModelFromSettings, resolveModelRoleValue } from "jeopi-cli/config/model-resolver";
...
describe("issue #980 provider-qualified model resolution", () => {
	...
	test("does not silently fall back to bedrock when a provider-qualified role misses", () => {
		...
		expect(cliResolved.error).toBe(
			'Model "anthropic/claude-3-7-sonnet" not found. Run "jeopi models" to see available models.',
		);
	});
});
