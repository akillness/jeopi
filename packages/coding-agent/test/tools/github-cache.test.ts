 ...
 *
 * Each test isolates `JEOPI_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "jeopi-cli/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "jeopi-cli/tools/github-cache";
import { ToolAbortError, throwIfAborted } from "jeopi-cli/tools/tool-errors";
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.JEOPI_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
