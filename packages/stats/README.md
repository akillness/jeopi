...
- **Session log parsing**: Reads JSONL session logs from `~/.jeopi/agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "jeopi-stats";
...
- **Session logs**: `~/.jeopi/agent/sessions/` (JSONL files)
- **Stats database**: `~/.jeopi/stats.db` (SQLite)
