...
import { fetchCursorUsableModels } from "jeopi-catalog/discovery/cursor";
import { GetUsableModelsResponseSchema, ModelDetailsSchema } from "jeopi-catalog/discovery/cursor-gen/agent_pb";
import { resolveProviderModels } from "jeopi-catalog/model-manager";
import { cursorModelManagerOptions } from "jeopi-catalog/provider-models/special";
...
async function createTempCachePath(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jeopi-cursor-cache-"));
	tempDirs.add(dir);
