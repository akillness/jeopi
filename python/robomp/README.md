...
`~/.jeopi/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host jeopi doesn't route through the gateway). roboomp lives inside the oh-my-pi
