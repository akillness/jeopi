...
Self-hosted GitHub triage bot. Drives [`jeopi --mode rpc`](https://github.com/akillness/jeopi)
...
Follow-up issue comments and PR review comments resume the same jeopi session
...
- **robomp** — FastAPI + sqlite event queue + `WorkerPool` running `jeopi` in
...
→ `worker.run_task` spawns `jeopi --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBOMP_MODEL` (CSV).

The agent uses jeopi's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.jeopi/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host jeopi doesn't route through the gateway). roboomp lives inside the jeopi
...
`PI_ROOT` only if you want a different jeopi checkout backing the build
and runtime.
...
runtime layer; editing pi source rebuilds `jeopi/pi:dev`, which
...
The integration test spawns a real `jeopi --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `jeopi` on
...
| Container exits with `PI_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/robomp/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid jeopi checkout. |
...
| `No API key found for <provider>` | `~/.jeopi/agent/models.container.yml` mount missing or provider id mismatch with `ROBOMP_MODEL`. |
...
src/
  ...
  worker.py          synchronous jeopi RPC driver, prompt assembly, env scrubbing
