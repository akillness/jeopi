...
Runs only when ROBOMP_INTEGRATION=1 and `jeopi` is available on PATH (or via
ROBOMP_JEOPI_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `jeopi --mode rpc` subprocess driven by `worker.run_task`.
...
pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBOMP_INTEGRATION=1 required to run the jeopi-backed smoke test",
)
