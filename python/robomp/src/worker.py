...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".jeopi/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".jeopi":
            # ~/.jeopi/run is slot-writable daemon presence state, not template
            # config; keep it out of the read-only normalization below.
            dirs[:] = [directory for directory in dirs if directory != "run"]
        try:
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.jeopi/run`` writable by every sandbox slot.
