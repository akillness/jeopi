...
The SDK is the in-process integration surface for `jeopi`.
...
`jeopi` exports the SDK APIs from the package root (and also via `jeopi/sdk`).
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.jeopi/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "jeopi-cli";
...
```ts
import { SessionManager } from "jeopi-cli";
