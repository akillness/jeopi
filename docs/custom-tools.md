...
1. Capability providers (`toolCapability`), including:
   - Native OMP config (`~/.jeopi/agent/tools`, `.jeopi/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.jeopi/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "jeopi-cli";
