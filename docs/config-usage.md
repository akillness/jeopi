...
┌───────────────────────────────────────┐
│ 1) ~/.jeopi/agent, ~/.claude, ...       │
│ 2) <cwd>/.jeopi, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .jeopi before user .jeopi;
...
`CONFIG_DIR_NAME` is `.jeopi` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`omp --profile <name>`, the `--alias` shortcut, or `OMP_PROFILE` / `PI_PROFILE`) relocates the OMP user base. When a profile is active, every OMP-native user-level path written here as `~/.jeopi/agent/...` resolves to `~/.jeopi/profiles/<name>/agent/...` instead.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own OMP config, never the default profile's `~/.jeopi/agent`.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.jeopi`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.jeopi/agent` as shorthand for the active profile's agent directory.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.jeopi`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.jeopi/agent/config.yml`
...
1. Migrate from `~/.jeopi/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.jeopi` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.jeopi/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.jeopi/agent/skills`, without requiring the root `.jeopi` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.jeopi` lookup for project files, but the project `.jeopi` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.jeopi/skills/*/SKILL.md` and `~/.jeopi/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.jeopi` directory search (walk-up) and still requires the project `.jeopi` dir to be non-empty.
...
```text
# ~/.jeopi/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery uses the same project-then-user config directory pattern as `SYSTEM.md`: project `.jeopi/TITLE_SYSTEM.md` first, then user `~/.jeopi/agent/TITLE_SYSTEM.md` and the other supported config bases.
