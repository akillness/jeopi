...
At each primary turn end, `AdvisorRuntime` receives only the new transcript delta since the last advisor update. Deltas are rendered with `formatSessionHistoryMarkdown(..., { includeThinking: true, includeToolIntent: true, watchedRoles: true, expandPrimaryContext: true, demoteThinkingForModelId: <advisor's live model id> })`, so the advisor can review assistant reasoning as well as user-visible text, tool calls, and tool results.

`demoteThinkingForModelId` matters for Anthropic Fable/Mythos advisor targets: replaying the primary's thinking as plaintext labeled `_thinking:_ …` is a `reasoning_extraction` classifier-refusal trigger (`Refusal (reasoning_extraction): This request was blocked as it seems to violate Anthropic's Terms of Service …`). Passing the advisor's own model id renders thinking through `renderDemotedThinking` instead — the same classifier-safe markdown-italic disguise (`_Hmm. …_`) the wire layer uses for cross-model/cross-turn thinking replay — so a Fable/Mythos advisor never sends the literal trigger, even on its first turn. If some other payload shape still draws a `reasoning_extraction` refusal, `AdvisorRuntime` falls back to a one-shot reactive degrade: it strips thinking blocks entirely, re-primes, and resends — permanently, for the rest of the session (`advisor.includeThinking` opts out of sending thinking at all up front).
...
1. user level: `<active agent dir>/WATCHDOG.md` (`~/.jeopi/agent/WATCHDOG.md` by default; relocated by `PI_CODING_AGENT_DIR`)
...
Candidates in hidden owner directories are ignored unless the file is inside an `.jeopi` directory. This keeps unrelated dot-directory conventions from being picked up accidentally while still allowing `.jeopi/WATCHDOG.md`.
