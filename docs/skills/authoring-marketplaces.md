...
description: Use when creating a new jeopi marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---

# Authoring Marketplaces

A marketplace is a Git repository (or local directory) that contains a catalog file at either `.omp-plugin/marketplace.json` (preferred for jeopi-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
The catalog file lives at either `.omp-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. jeopi prefers the `.omp-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for jeopi"
  },
  "plugins": [
    {
      "name": "acme-linter",
...
> Note: extension modules declared via `package.json` `jeopi.extensions` are **not** loaded from marketplace installs — that mechanism only applies to npm-installed or `jeopi plugin link`ed plugins. Ship marketplace plugin behavior through the conventional directories above.
...
```
jeopi plugin marketplace add owner/repo
jeopi plugin install name@marketplace-name
```
...
1. Create `marketplace.json` at `.omp-plugin/marketplace.json` (jeopi-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
