...
This document defines the JS/TS contract between `jeopi-natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `jeopi-natives`.
