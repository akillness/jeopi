...
A **plugin** is a directory containing Claude/OMP plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `jeopi.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `jeopi plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.jeopi/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.jeopi/plugins/installed_plugins.json`
...
```
jeopi plugin marketplace add <source>
jeopi plugin marketplace remove <name>
jeopi plugin marketplace update [name]
jeopi plugin marketplace list
jeopi plugin discover [marketplace]
jeopi plugin install [--force] [--scope user|project] name@marketplace
jeopi plugin uninstall [--scope user|project] name@marketplace
jeopi plugin upgrade [--scope user|project] [name@marketplace]
jeopi plugin enable [--scope user|project] name@marketplace
jeopi plugin disable [--scope user|project] name@marketplace
```
...
A marketplace catalog lives at `.omp-plugin/marketplace.json` in the repository root. When jeopi is the only intended consumer, prefer this path. To remain Claude Code-compatible (jeopi loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — jeopi uses it as a fallback when `.omp-plugin/marketplace.json` is absent. A repository may ship both: jeopi reads the `.omp-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
