...
This document explains how token/tool streaming is normalized in `jeopi-ai`, then propagated through `jeopi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `jeopi-ai`
