...
```text
~/.jeopi/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.jeopi/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.jeopi/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.jeopi/agent/sessions`
