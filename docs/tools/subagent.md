...
10. `resume` requires `id`, rejects self-targeting. A registry `running` target is a no-op (nothing to resume). A `queued` job is a no-op (already waiting for a spawn slot). Any other target requires `message` (throws `ToolError` otherwise, since the underlying `AgentSession` has no pending work to continue on its own); on non-`failed` delivery, `#pausedIds` is cleared for `id`.
...
- `resume` on a target that is not `running`/`queued` and has no `message` throws `ToolError` — there is no implicit "just continue" for a finished agent.
