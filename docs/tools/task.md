...
- Key collaborators:
  ...
  - `jeopi-natives` (`crates/pi-iso`) — isolation PAL: `isoResolve` / `isoStart` / `isoStop` backend resolution and fallback.
...
- Agent discovery precedence is first-wins by exact name: project `.jeopi` agents dir before the user `.jeopi` dir (task agents only load from `.jeopi` roots; `.claude`/`.codex`/`.gemini` agent dirs are skipped), Claude plugin agent dirs after config dirs, bundled agents last. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
