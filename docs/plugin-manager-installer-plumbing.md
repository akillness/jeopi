...
omp plugin <npm/link action> ...
  ...
  -> mutate ~/.jeopi/plugins/{package.json,node_modules,omp-plugins.lock.json}
...
omp plugin install name@marketplace / omp install name@marketplace
  -> MarketplaceManager
  -> mutate ~/.jeopi/marketplaces.json, ~/.jeopi/plugins/installed_plugins.json, cache dirs
...
Global plugin state lives under `~/.jeopi/plugins`:
...
- `<cwd>/.jeopi/plugin-overrides.json`
...
- `~/.jeopi/marketplaces.json` — configured marketplace catalogs
- `~/.jeopi/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.jeopi/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.jeopi/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
...
4. Run `bun install <packageSpec>` in `~/.jeopi/plugins`.
...
1. Read plugin dependency map from `~/.jeopi/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.jeopi/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.jeopi/plugins/node_modules/<pkg.name>`.
