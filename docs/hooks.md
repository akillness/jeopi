...
- JS/TS hook factories discovered through `hookCapability` (for example `.jeopi/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
...
```ts
import type { HookAPI } from "jeopi-cli/extensibility/hooks";
...
`src/extensibility/hooks/index.ts` and the package subpath `jeopi/extensibility/hooks` export:
...
The package root (`jeopi`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
