...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from OMP-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the OMP task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".jeopi"` filters both dir lists).

### Discovery inputs

1. Nearest project `.jeopi` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.jeopi`; first hit only)
2. User `.jeopi` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.jeopi`; first hit only)
...
1. project `.jeopi/agents`
2. user `~/.jeopi/agent/agents`
...
- Project `.jeopi` overrides user `.jeopi`.
