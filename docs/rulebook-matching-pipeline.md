...
Loads `.jeopi` rules from:

- project: `<cwd>/.jeopi/rules/*.{md,mdc}` when the cwd `.jeopi` directory exists
- user: `~/.jeopi/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.jeopi/agent/RULES.md`
- sticky project rule: nearest ancestor `.jeopi/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.jeopi/rules`, user `~/.jeopi/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
