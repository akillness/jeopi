...
const SUMS = {
	"jeopi-darwin-arm64": "darwin_arm64_sha",
	"jeopi-darwin-x64": "darwin_x64_sha",
	"jeopi-linux-arm64": "linux_arm64_sha",
	"jeopi-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		...
		for (const arch of ["jeopi-darwin-arm64", "jeopi-darwin-x64", "jeopi-linux-arm64", "jeopi-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.jeopi` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"jeopi", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
