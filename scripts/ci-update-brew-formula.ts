...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// not touch the real `/Users/<user>/.jeopi` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Jeopi < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/jeopi-darwin-arm64",
          using: :nounzip
      sha256 "${sums["jeopi-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/jeopi-darwin-x64",
          using: :nounzip
      sha256 "${sums["jeopi-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/jeopi-linux-arm64",
          using: :nounzip
      sha256 "${sums["jeopi-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/jeopi-linux-x64",
          using: :nounzip
      sha256 "${sums["jeopi-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["jeopi-*"].first => "jeopi"
    (bin/"jeopi").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"jeopi", "completions", shells: [:bash, :zsh, :fish])
    end
  end
...
async function main(): Promise<void> {
	...
	const targets = ["jeopi-darwin-arm64", "jeopi-darwin-x64", "jeopi-linux-arm64", "jeopi-linux-x64"];
