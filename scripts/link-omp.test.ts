...
describe("scripts/link-omp.sh", () => {
	it("falls back to BUN_INSTALL/bin when bun cannot resolve the global bin", () => {
		...
		expect(fs.readlinkSync(path.join(bunInstall, "bin", "jeopi"))).toBe(targetWrapper);
	});
	...
	it("uses bun pm -g bin when it succeeds", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stderr).toBe("");
		expect(fs.readlinkSync(path.join(globalBin, "jeopi"))).toBe(targetWrapper);
		expect(fs.existsSync(path.join(dir, "unused-bun-install", "bin", "jeopi"))).toBe(false);
	});
});
