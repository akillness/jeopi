...
const FALLBACK_REPO = "akillness/jeopi";
...
function placeholderManifest(name: string, target: NativeLeafTarget, repo: string): PlaceholderManifest {
	return {
		...
		description: `Placeholder for the ${target.tag} native addon of jeopi-natives. The real binary is published during release.`,
		license: "MIT",
		os: [target.os],
		cpu: [target.cpu],
		repository: {
...
function placeholderReadme(name: string, target: NativeLeafTarget): string {
	return [
		`# ${name}`,
		"",
		`Placeholder package reserving the npm name for the \`${target.tag}\` native addon of \`jeopi-natives\`.`,
		"",
