...
| 8 | v16.5.1 | 14b5da76a | 431 | +190 | in progress: 34/190 ported (largest checkpoint yet — first checkpoint with many small external-contributor PR fixes rather than large features; each real change is a paired `fix(...)` + redundant `Merge PR #NNNN` commit, only the `fix(...)` carries a unique diff) |
...
### Checkpoint 8 — v16.5.1 (190 commits) — IN PROGRESS (34/190 ported)
...
**Ported (34 upstream commits represented by 27 jeopi commits):**
